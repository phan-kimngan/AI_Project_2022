...
def app():
    ...
    with header:
        ...
        if video:
            org_link = "./data/outputs_test/video_recorder/"
            link = "./data/outputs2/video_recorder/"
        if webcam:
            org_link = "./data/outputs_test/webcam/"
            link = "./data/outputs2/webcam/"
        if video or webcam:
            ...
            for folder in link_list:
                ...
                if k == str(date):
                    ...
                    if folders[folder]:
                        items = os.listdir(link+folder+"/")
                        for item in items:
                            if item.split(".")[1] == "mp4":
                                org_video_file = org_link+folder+"/"+item
                                video_file = link+folder+"/"+item
